...
** TODO
*** TODO use the clipboard, rather than the active layer, as the piece source
This is done so that the source need not be the active layer, since the
script will automatically create a new collage layer that will match the size of
the active layer.  Alternatively, the user could be prompted for a layer size.

Maybe checkboxes and options could be present for each:
**** clipboard or active layer as source
**** new layer size matches active layer or uses a user-specified size
** LICENSE
